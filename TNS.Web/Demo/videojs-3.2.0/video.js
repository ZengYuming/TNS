...
(function (window, undefined) {
    ...
    _V_.log = function () {
        ...
        if (window.console) {
            ...
            var newarr = [].slice.call(arguments); (typeof console.log === "object" ? _V_.log.apply.call(console.log, console, newarr) : console.log.apply(console, newarr))
        }
    }; (function (b) {
        function c() { }
    ...
    _V_.Component = _V_.Class.extend({
        init: function (player, options) {
            this.player = player;
            options = this.options = _V_.merge(this.options || {},
            options);
            if (options.el) {
                this.el = options.el
            } else {
        ...
        initComponents: function () {
            ...
            if (options && options.components) {
                this.eachProp(options.components,
                function (name, opts) {
    ...
    _V_.Player = _V_.Component.extend({
        init: function (tag, addOptions, ready) {
            ...
            if (!options.sources || options.sources.length == 0) {
                for (var i = 0,
                j = options.techOrder; i < j.length; i++) {
                    var techName = j[i],
                    tech = _V_[techName];
                    if (tech.isSupported()) {
        ...
        getVideoTagSettings: function () {
            ...
            if (this.tag.hasChildNodes()) {
                for (var c, i = 0,
                j = this.tag.childNodes; i < j.length; i++) {
                    c = j[i];
        ...
        selectSource: function (sources) {
            for (var i = 0,
            j = this.options.techOrder; i < j.length; i++) {
                var techName = j[i],
                tech = _V_[techName];
                if (tech.isSupported()) {
                    for (var a = 0,
    ...
    _V_.flash = _V_.PlaybackTech.extend({
        init: function (player, options) {
            ...
            if (options.iFrameMode == true && !_V_.isFF) {
                ...
                _V_.addEvent(iFrm, "load", _V_.proxy(this,
                function () {
                    ...
                    iWin.ready = _V_.proxy(this.player,
                    function (currSwf) {
                    ...
                    iWin.events = _V_.proxy(this.player,
                    function (swfID, eventName, other) {
                        var player = this;
                        if (player && player.techName == "flash") {
                            player.triggerEvent(eventName)
                        }
                    });
                    iWin.errors = _V_.proxy(this.player,
    ...
    _V_.TextTrackButton = _V_.Button.extend({
        ...
        createItems: function () {
            var items = [];
            this.each(this.player.textTracks,
            function (track) {
